...
(function (JSZip, html2canvas) {
  ...
  async function fetchAllNonProjectConversations(projectIds, maxConversations = 1e3, onBatch, onHasMore) {
    ...
    while (conversations.length < maxConversations) {
      try {
        ...
        const matching = result.items.filter((conversation) => !conversation.gizmo_id || !knownProjectIds.has(conversation.gizmo_id));
        ...
        if (accepted.length > 0) onBatch == null ? void 0 : onBatch(accepted);
        const nextOffset = offset + result.items.length;
        const apiHasMore = result.total !== null ? nextOffset < result.total : result.items.length >= limit;
        ...
        console.error("Error fetching non-project conversations batch:", error2);
        break;
      }
    }
    onHasMore == null ? void 0 : onHasMore(hasMore);
  ...
  const en_US = {
    ...
    "Not in a project": "Not in a project",
  ...
  const es = {
    ...
    "Not in a project": "No está en un proyecto"
  };
  ...
  const fr = {
    ...
    "Not in a project": "Pas dans un projet"
  };
  ...
  const id_ID = {
    ...
    "Not in a project": "Tidak dalam proyek"
  };
  ...
  const tr_TR = {
    ...
    "Not in a project": "Bir projede değil"
  };
...
${content2}`;
  ...
  function classifyFileReference(reference) {
    const rawText = [
      ...
      typeof reference.rawValue === "string" ? reference.rawValue : void 0
    ].filter(Boolean).join(" ");
  ...
  function isSandboxReference(reference) {
    const values = [
      reference.rawPath,
      reference.assetPointer,
      reference.rawUrl,
    ...
    return values.some(
      (value) => typeof value === "string" && (value.startsWith("sandbox:/mnt/data/") || value.startsWith("/mnt/data/") || value.includes("sandbox:/mnt/data/") || value.includes("/mnt/data/"))
    );
  }
  function isCitationUrl(reference) {
    var _a, _b, _c;
    return ((_a = reference.sourceField) == null ? void 0 : _a.includes("citation")) === true || ((_b = reference.sourceField) == null ? void 0 : _b.includes("content_references")) === true || ((_c = reference.sourceField) == null ? void 0 : _c.includes("_cite_metadata")) === true;
  }
  function isSearchResultUrl(reference) {
    var _a, _b, _c, _d;
    return ((_a = reference.sourceField) == null ? void 0 : _a.includes("search_result")) === true || ((_b = reference.sourceField) == null ? void 0 : _b.includes("aggregate_result")) === true || ((_c = reference.sourceField) == null ? void 0 : _c.includes("webpage")) === true || ((_d = reference.rawUrl) == null ? void 0 : _d.startsWith("http")) === true;
  }
  function isTerminalOrExecutionText(text2) {
    return /askmr@DESKTOP|PowerShell|Traceback|Exception|Line \||^\s*at\s+/m.test(text2);
  }
  function isLikelyConversationContent(reference, text2) {
    if (reference.authorRole === "assistant" && !reference.fileId && !reference.rawPath && !reference.rawUrl) {
      return true;
    }
    return text2.length > 120 && !text2.includes("/files/download/") && !text2.includes("sandbox:/mnt/data/") && !text2.includes("/mnt/data/");
  }
  ...
  const DialogContent = ({ format }) => {
    ...
    p$6(() => {
      const off = requestQueue.on("done", async (results) => {
        ...
        if (callback) {
          await callback(format, results, metaList, selectedProjectName, partIndex, totalBatches2);
        }
    ...
    const exportAllFromLocal = T$4(async () => {
      ...
      for (let i2 = 0; i2 < chunks.length; i2++) {
        await callback(format, chunks[i2], metaList, selectedProjectName, i2 + 1, chunks.length);
    ...
    p$6(() => {
      ...
      fetchProjects().then(setProjects).catch((err) => console.error("Error fetching projects:", err)).finally(() => {
    ...
    p$6(() => {
      ...
      const onHasMore = (hasMore2) => {
        if (alive()) setHasMore(hasMore2);
      };
      const request = selectedProjectId === null ? fetchAllConversationsAll(projects, exportAllLimit, onBatch) : isNotInProject ? fetchAllNonProjectConversations(projectIds, exportAllLimit, onBatch, onHasMore) : fetchAllConversations(selectedProjectId, exportAllLimit, onBatch, onHasMore);
      request.catch((err) => {
        if (!alive()) return;
